...
#undef min
	...
	inline void vector<T>::emplace_back(Args && ...args) {
		if (end_ == cap_) {
			...
			data_allocator::construct(&*end_, esgstl::forward<Args>(args)...);
			++end_;
		}
	}
	...
	inline void vector<T>::copy_insert(iterator pos, Iter first, Iter last) {
		...
		if (size() > capacity() - n) {
			...
			if (after_elems > n) {
				...
				// 扩充的空间长度大于pos后面元素的个数，因此新扩充的空间前面全是value
				auto mid = first;
				esgstl::advance(mid, after_elems);
				end_ = esgstl::uninitialized_copy(mid, last, end_);
				end_ = esgstl::uninitialized_move(pos, old_end, end_);
				esgstl::uninitialized_copy(first, mid, pos);
			}
		}
	}
